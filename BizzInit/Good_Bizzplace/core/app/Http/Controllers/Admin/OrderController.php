...
class OrderController extends Controller
{
    ...
    public function shippingchange(Request $request) {
      ...
      if ($order->shipping_status == 1) {
        ...
        elseif ($order->shipping_method == 'around') {
          ...
          foreach ($order->orderedproducts as $key => $op) {
            if (!in_array($op->vendor->id, $sentVendors)) {
              $sentVendors[] = $op->vendor->id;
              send_email($op->vendor->email, $op->vendor->shop_name, 'La livraison du produit est en cours', "Merci d'avoir envoyé vos produits. Nous enverrons ces produits au client par courrier dans un délai de ".$gs->am_min." à ".$gs->am_max." days.<p><strong>Numéro commande: </strong>".$order->unique_id."</p> <p><strong>Details commandes: </strong><a href='".url('/')."/vendor"."/".$order->id."/orderdetails'>".url('/')."/vendor"."/".$order->id."/orderdetails"."</a></p>");
            }
          }
        ...
        elseif ($order->shipping_method == 'world') {
          ...
          foreach ($order->orderedproducts as $key => $op) {
            if (!in_array($op->vendor->id, $sentVendors)) {
              $sentVendors[] = $op->vendor->id;
              send_email($op->vendor->email, $op->vendor->shop_name, 'La livraison du produit est en cours', "Merci d'avoir envoyé vos produits. Nous enverrons ces produits au client par courrier dans un délai de ".$gs->aw_min." to ".$gs->aw_max." days.<p><strong>Numéro commande: </strong>".$order->unique_id."</p> <p><strong>Details commandes: </strong><a href='".url('/')."/vendor"."/".$order->id."/orderdetails'>".url('/')."/vendor"."/".$order->id."/orderdetails"."</a></p>");
            }
          }
          // sending mail to user
          send_email($order->user->email, $order->user->first_name, 'La livraison du produit est en cours', "Votre livraison de produit est en cours. Nous avons collecté les produits des fournisseurs et vous les enverrons par courrier au plus tard ".$gs->aw_min." to ".$gs->aw_max." days.<p><strong>Numéro commande: </strong>$order->unique_id</p><p><strong>Details commandes: </strong><a href='".url('/')."/".$order->id."/orderdetails'>".url('/')."/".$order->id."/orderdetails"."</a></p>");
          send_sms($order->user->phone, "Votre livraison de produit est en cours. Nous avons collecté les produits des fournisseurs et vous les enverrons par courrier au plus tard ".$gs->aw_min." to ".$gs->aw_max." days.<p><strong>Numéro commande: </strong>$order->unique_id</p><p><strong>Details commandes: </strong><a href='".url('/')."/".$order->id."/orderdetails'>".url('/')."/".$order->id."/orderdetails"."</a></p>");
      ...
      elseif ($order->shipping_status == 2) {
        ...
        foreach ($order->orderedproducts as $key => $op) {
          if (!in_array($op->vendor->id, $sentVendors)) {
            $sentVendors[] = $op->vendor->id;
            send_email($op->vendor->email, $op->vendor->shop_name, 'Produits livrés', "Merci de vous envoyer des produits. Nous avons livré vos produits au client.<p><strong>Numéro commande: </strong>".$order->unique_id."</p> <p><strong>Details commandes: </strong><a href='".url('/')."/vendor"."/".$order->id."/orderdetails'>".url('/')."/vendor"."/".$order->id."/orderdetails"."</a></p>");
          }
        }

        // sending mail to user
        send_email($order->user->email, $order->user->first_name, 'Produits livrés', "Merci d'avoir choisi <strong>".$gs->website_title."</strong> pour le shopping. Nous avons constaté que vous aviez reçu la livraison des produits souhaités. S'il vous plaît donner un avis / suggestion afin que nous puissions améliorer la qualité de nos produits.<p><strong>Numéro commande: </strong>$order->unique_id</p><p><strong>Details commandes: </strong><a href='".url('/')."/".$order->id."/orderdetails'>".url('/')."/".$order->id."/orderdetails"."</a></p>");
    ...
    public function acceptOrder(Request $request) {
      ...
      foreach ($order->orderedproducts as $key => $op) {
        if (!in_array($op->vendor->id, $sentVendors)) {
          $sentVendors[] = $op->vendor->id;
          send_email($op->vendor->email, $op->vendor->shop_name, 'Commande acceptée', "Order ID #".$order->unique_id." a été accepté.<p><strong>Details commandes: </strong><a href='".url('/')."/vendor"."/".$order->id."/orderdetails'>".url('/')."/vendor"."/".$order->id."/orderdetails"."</a></p>");
        }
      }
      // sending mail to user
      send_email($order->user->email, $order->user->first_name, 'Commande acceptée', "Votre commande a été accepté.<p><strong>Numéro commande: </strong>$order->unique_id</p><p><strong>Details commandes: </strong><a href='".url('/')."/".$order->id."/orderdetails'>".url('/')."/".$order->id."/orderdetails"."</a></p>");
    ...
    public function cancelOrder(Request $request) {
      ...
      foreach ($order->orderedproducts as $key => $op) {
        if (!in_array($op->vendor->id, $sentVendors)) {
          $sentVendors[] = $op->vendor->id;
          send_email($op->vendor->email, $op->vendor->shop_name, 'Ordre rejeté', "Order ID #".$order->unique_id." a été refusé.<p><strong>Details commandes: </strong><a href='".url('/')."/vendor"."/".$order->id."/orderdetails'>".url('/')."/vendor"."/".$order->id."/orderdetails"."</a></p>");
        }
      }
      // sending mail to user
      send_email($order->user->email, $order->user->first_name, 'Ordre rejeté', "Votre commande a été refusé.<p><strong>Numéro commande: </strong>$order->unique_id</p><p><strong>Details commandes: </strong><a href='".url('/')."/".$order->id."/orderdetails'>".url('/')."/".$order->id."/orderdetails"."</a></p>");
