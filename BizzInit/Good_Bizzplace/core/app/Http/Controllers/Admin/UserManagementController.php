...
class UserManagementController extends Controller
{
    ...
    public function updateUserDetails (Request $request) {
      ...
      Session::flash('success', 'Les détails de l\'utilisateur ont été mis à jour avec succès!');
    ...
    public function updateUserBalance(Request $request) {
      ...
      if ($request->has('operation')) {
        ...
        $successMessage = 'Le montant a été ajouté avec succès!';
      } else {
        $balance = $balance - $request->amount;
        $successMessage = 'Le montant a été soustrait avec succès!';
      }
      if($request->has('message')) {
        $name = $user->name;
        $subject = 'Solde mis à jour dans votre compte';
    ...
    public function ads(Request $request, $userId) {
      if (empty($request->term)) {
        $data['term'] = '';
        $data['items'] = Item::where('user_id', $userId)->latest()->paginate(9);
      } else {
        $data['term'] = $request->term;
        $data['items'] = Item::where('user_id', $userId)->where('title', 'like', '%'.$request->term.'%')->latest()->paginate(9);
      }
    ...
    public function mailtosubsc(Request $request) {
      ...
      Session::flash('success', 'Courrier envoyé à tous les abonnés.');
      return redirect()->back();
    }
}
