...
@section('content')
  <main class="app-content">
     <div class="app-title">
        <div>
           <h1>Sous-catégorie sous <strong>{{$subcategory->name}}</strong> Categorie</h1>
        </div>
     </div>
     <div class="row">
        <div class="col-md-12">
                     ...
                     <tbody>
                          @foreach ($subcats as $key => $subcat)
                            <tr>
                               ...
                               <td>
                                 <a href="{{route('admin.subcategory2.index', $subcat->id)}}" class="btn btn-info"><i class="fa fa-eye"></i> consulter</a>
                               </td>
                               <td>
                            ...
                            @includeif('admin.subcategory1.partials.edit')
                          @endforeach
                     </tbody>
                  </table>
                @endif
  ...
  @includeif('admin.subcategory1.partials.add')
@endsection
@push('scripts')
  <script>
    ...
    function deletesub(e, pid) {
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            url: '{{route('admin.subcategory1.destroy')}}',
            type: 'POST',
            data: {
              id: pid
            },
