                                          ...
                                          <tr>
                                              ...
                                              <td class="padding-top-40">
                                               <div class="form-check form-check-inline">
                                                <input class="form-check-input " type="radio" name="shipping{{$order->id}}" id="inlineRadio{{$order->unique_id}}1" value="0" onchange="shippingChange(event, this.value, {{$order->id}})" {{$order->shipping_status==0 || $order->approve==0 ?'checked':''}} disabled>
                                                 <label class="form-check-label" for="inlineRadio{{$order->id}}1">En attente</label>
                                               </div>
                                               <div class="form-check form-check-inline">
                                                 <input class="form-check-input" type="radio" name="shipping{{$order->id}}" id="inlineRadio{{$order->unique_id}}2" value="1" onchange="shippingChange(event, this.value, {{$order->id}})" {{$order->shipping_status==1 || $order->approve==1 ?'checked':''}} {{$order->shipping_status==1 || $order->shipping_status==2?'disabled':''}} disabled>
                                                 <label class="form-check-label" for="inlineRadio{{$order->id}}2">En cours</label>
                                               </div>
                                               <div class="form-check form-check-inline">
                                                  <input class="form-check-input" type="radio" name="shipping{{$order->id}}" id="inlineRadio{{$order->unique_id}}3" value="2" onchange="shippingChange(event, this.value, {{$order->id}})" {{$order->shipping_status==2 ?'checked':''}} {{$order->shipping_status==2?'disabled':''}} disabled>
                                              ...
                                              @if ($order->approve == 0)
                                                <span>
                                                  <a href="#" onclick="cancelOrder(event, {{$order->unique_id}})" title="Rejeter commande">
                                                    <i class="fa fa-times text-danger"></i>
                                                  </a>
                                                  <a href="#" onclick="acceptOrder(event, {{$order->unique_id}})" title="Accepter commande">
        ...
        if (willDelete) {
          $.ajax({
            ...
            success: function(data) {
              console.log(data);
              if (data == "success") {
                $('#inlineRadio'+orderid).removeAttr('disabled');
                window.location = '{{url()->full()}}';
              }
            }
          });
