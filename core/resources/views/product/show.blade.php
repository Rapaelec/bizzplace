...
@section('headertxt', 'Detail Produit')
                          ...
                          <form class="bottom-content" id="attrform" enctype="multipart/form-data">
                            {{csrf_field()}}
                              ...
                              <ul class="product-spec">
                                  ...
                                  <li>Code Produit: <span class="right">{{$product->product_code}}</span></li>
                                  @if ($product->quantity > 0)
                                    <li>Stock:  <span class="right base-color" id="stock">En Stock </span></li>
                                  @else
                                    <li>Stock:  <span class="right text-danger" id="stock">En rupture de stock </span></li>
                                  @endif
                                  ...
                                  @if($attr!=null)
                                    ...
                                    @endif
                                  @endforeach
                                  <li>Nom de la boutique:  <span class="right"><a href="{{route('vendor.shoppage', $product->vendor->id)}}" style="color:#{{$gs->base_color_code}};font-weight:700;">{{$product->vendor->shop_name}}</a> </span></li>
                              ...
                              <div class="paction">
                                  ...
                                  @if (!Auth::guard('vendor')->check())
                                    <div class="checkbox-wrapper mt-3">
                                        <label class="checkbox-inner">J'ai lu et accepté les <a href="{{route('terms')}}" class="base-color">Termes et Conditions du vendeur</a>
                                    ...
                                    <div class="btn-wrapper mt-1">
                                        <a href="#" class="btn btn-danger" @click.prevent="addtocart({{$product->id}}, $refs.qttotal.innerHTML)"><i class="fa fa-shopping-cart" aria-hidden="true"></i> AJOUTER AU PANIER</a>
                                        <a href="#" class="btn btn-success" data-toggle="modal" data-target="#sendMessageVendor"><i class="fa fa-paper-plane" aria-hidden="true"></i> ENVOYER UN MESSAGE</a>
                                    </div>
                                  @endif
                              </div>
                          </form>
                      ...
                      <div class="product-details-area">
                          <div class="product-details-tab-nav">
                              <ul class="nav nav-tabs" role="tablist">
                                  ...
                                  <li class="nav-item">
                                    <a class="nav-link {{session('success')=='Reviewed successfully'?'active':''}}" id="item-review-tab" data-toggle="tab" href="#item_review" role="tab" aria-controls="item_review" aria-selected="true">Avis Article</a>
                                  </li>
                                  @auth
                                  ...
                                  @endauth
                                  <li class="nav-item">
                                    <a class="nav-link" id="item-review-tab" data-toggle="tab" href="#vendor_info" role="tab" aria-controls="item_review" aria-selected="true">Information Vendeur</a>
                                  </li>
                              </ul>
                          </div>
                  ...
                  <div class="recently-added-carousel" id="recently-added-carousel"><!-- recently added carousel -->
                    @foreach ($rproducts as $key => $rproduct)
                      <div class="single-new-collection-item "><!-- single new collections -->
                        <span class="badge badge-info mb-1" style="margin-left: 65%; font-size: 91%;"><i class="fa fa-eye"></i> Vues {{$rproduct->number_views}}</span>  
                          <div class="thumb">
                              <img src="{{asset('assets/user/img/products/'.$rproduct->previewimages()->first()->image)}}" alt="new collcetion image">
                              <div class="hover">
                                <a href="{{route('user.productdetail.show', [$rproduct->slug, $rproduct->id])}}" class="view-btn"><i class="fa fa-eye"></i></a>
                              </div>
                          </div>
                          <div class="content">
                              <span class="category">{{\App\Category::find($rproduct->category_id)->name}}</span>
                              <a href="{{route('user.productdetail.show', [$rproduct->slug, $rproduct->id])}}"><h4 class="title">{{strlen($rproduct->title) > 25 ? substr($rproduct->title, 0, 25) . '...' : $rproduct->title}}</h4></a>
                    ...
                    <div class="single-new-collection-item ">
                      <div class="view-all-wrapper">
                        <div class="view-all-inner">
                          ...
                          <a class="d-block view-all-txt" href="{{route('user.search', [$rproduct->category_id, $rproduct->subcategory_id])}}">Voir tout</a>
                        </div>
                      </div>
                    </div>
