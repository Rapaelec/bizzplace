          ...
          <div class="col-md-4">
            <div class="card">
              ...
              <ul class="list-group list-group-flush">
                <li class="list-group-item"><strong>LIMITE </strong>({{$gateway->minamo}} - {{$gateway->maxamo}}) {{$gs->base_curr_text}}</li>
                <li class="list-group-item"><strong>CHARGE</strong> {{$gateway->fixed_charge}} {{$gs->base_curr_text}} + {{$gateway->percent_charge}}  %</li>
              </ul>
              <div class="card-body">
                <button class="btn btn-block btn-primary" type="button" name="button" data-toggle="modal" data-target="#depositModal{{$gateway->id}}">Déposer maintenant</button>
              </div>
            </div>
          </div>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
              <form class="modal-content" method="post" action="{{route('vendor.depositDataInsert')}}" enctype="multipart/form-data">
                <div class="modal-header">
                  <h5 class="modal-title" id="exampleModalLabel">Déposer via {{$gateway->name}}</h5>
                  ...
                  {{-- <form method="post" action="{{route('user.deposit.preview')}}"> --}}
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group mb-3">
                        <input name="amount" type="text" class="form-control" placeholder="Entre le Montant" required>
                      ...
                      <div class="fileinput fileinput-new" data-provides="fileinput">
                          ...
                          <div>
                              <span class="btn btn-success btn-file">
                                  <span class="fileinput-new"> Image du reçu </span>
