<div class="comments">
  ...
  @if (!empty($product->productreviews) && $product->productreviews()->count() == 0)
    <div class="card">
      <div class="card-body">
        <h3 class="text-center base-txt">Pas encore d'avis ...</h3>
      </div>
    </div>
  @else
    <div class="row">
      <div class="col-md-12 text-center">
        <h2 class="mt-2" style="color:#f0932b">{{round(\App\ProductReview::where('product_id', $product->id)->avg('rating'), 2)}}/5.0</h2>
           Basé sur {{\App\ProductReview::where('product_id', $product->id)->count()}} avis
      </div>
    </div>
