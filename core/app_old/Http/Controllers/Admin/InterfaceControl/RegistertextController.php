...
class RegistertextController extends Controller
{
    ...
    public function update(Request $request) {
      $request->validate([
        ...
        'entreprise_register_text' => 'required',
      ]);

      $gs = GS::first();
      $in = $request->only('user_register_text', 'vendor_register_text','entreprise_register_text');
      $gs->fill($in)->save();

      Session::flash('success', 'Texte mis à jour avec succès');
      return back();
    }
}
