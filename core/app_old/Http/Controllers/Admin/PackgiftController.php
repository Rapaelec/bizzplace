...
class PackgiftController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        if($request->hasFile('img_pack')) {
            $imgpackgift_full_name = time().'.'.$imgpackgift->getClientOriginalExtension();
            $image = $request->file('img_pack');
            $location = 'assets/user/packgift/img' . $imgpackgift_full_name;
            $package->img_pack =  $location;
      ...
      public function update(Request $request) {
        $rules = [
            ...
            'price'=>'required',
            'validity'=>'required',
            'img_pack'=>'required|image|bail|nullable|max:10000'
        ];
