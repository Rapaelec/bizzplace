...
class GatewayController extends Controller {
    ...
    public function vendorDataUpdate($data) {
        if ($data->status == 0) {
            ...
            $message = "Votre commande a été passée avec succès! Notre agent vous contactera plus tard. <br><strong>Commande ID: </strong> " . $data->order->unique_id ."<p><strong>Details Commande: </strong><a href='".url('/')."/".$data->order_id."/orderdetails'>".url('/')."/".$data->order_id."/orderdetails"."</a></p>";

            send_email($data->user->email, $data->user->first_name,$data->user->phone,$data->user->email, "Commande éffectuée", $message);
            send_sms( $data->user->phone, $message);            

            Session::flash('success', 'Commande passée avec succès! Notre agent vous contactera plus tard.');
    ...
    public function paymentConfirm() {
        ...
        if ($data->gateway_id == 101) {
            ...
            $vendorOrders = Orderedproduct::where('order_id',$data->order_id)->get();
            ...
            if ($data->btc_amo == 0 || $data->btc_wallet == "") {
                ...
                if (!$resp) {
                    return redirect()->route('user.gateways')->with('alert', 'L\'API BLOCKCHAIN AYANT UN PROBLÈME. MERCI D\'ESSAYER PLUS TARD');
                }
            ...
            if ($btcdata->status != 'success') {
                return back()->with('alert', 'Processus échoué');
            }
            ...
            if ($order) {
                ...
                return redirect()->route('user.gateways')->with('alert', 'Erreur inattendue! Veuillez réessayer');
                exit();
            }
    ...
    public function paymentConfirmFormule() {
        $gnl = GeneralSetting::first();
        $track = Session::get('Track');
        $data = Orderpayment::where('trx', $track)->orderBy('id', 'DESC')->first();
        // return $data;
    ...
    public function ipnpaypal()
    {
        ...
        $verify = file_get_contents($callUrl);
            ...
            if($receiver_email==$gatewayData->val1 && $mc_currency=="EUR" && $mc_gross ==$amount && $data->status=='0')
            {
                //Update User Data
        ...
        try
        {
            ...
            try
            {
                ...
                if ($charge['status'] == 'succeeded') {
                    ...
                    //Update User Data
                    $this->vendorDataUpdate($data);
                    return redirect()->route('user.orders')->with('success', 'Commande passée avec succès! Notre agent vous contactera plus tard.');
        ...
        if(verifychecksum_e($_POST, $paytm_merchant_key, $_POST['CHECKSUMHASH']) === "TRUE") {
            ...
            if($_POST['RESPCODE'] == "01"){
                ...
                if($responseParamList['STATUS'] == 'TXN_SUCCESS' && $responseParamList['TXNAMOUNT'] == $_POST['TXNAMOUNT']) {
                    ...
                    $t = 'success';
                    $m = 'Commande passée avec succès! Notre agent vous contactera plus tard.';
                } else  {
                    $t = 'alert';
                    ...
                    if (!$err)
                    {
                        switch ($_POST['m_status'])
                        {
                            case 'success':
                                ...
                                $this->vendorDataUpdate($ddd);
                                $message = 'Commande passée avec succès! Notre agent vous contactera plus tard.';
                                $err = false;
    ...
    public function purchaseVogue($trx, $type)
    {
        ...
        return redirect()->route('user.orders')->with('success', 'Commande passée avec succès! Notre agent vous contactera plus tard., Ref: ' . $trx);
    }
