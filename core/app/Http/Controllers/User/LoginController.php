    ...
    public function authenticate(Request $request) {
        if (Auth::check()) {
          if(Auth::user()->reason_social!=''){
            Session::flash('alert', "Vous êtes déjà connecté en tant qu'entreprise");
          }
          else{
            Session::flash('alert', "Vous êtes déjà connecté en tant qu'utilisateur");
          }
          return back();
        }
        ...
        $validatedRequest = $request->validate([
          ...
          // dd($validatedRequest);
        $user = User::where('username', $request->utilisateur)->first();
        if (Auth::attempt([
