...
class DepositController extends Controller
{
    public function showDepositMethods() {
      $data['gateways'] = Gateway::where('status', 1)->first();
    ...
    public function depositDataInsert(Request $request)
    {
        ...
        $validator = Validator::make($request->all(),[
            ...
            if ($validator->fails()) {
                return back()->with('alert',$validator);
                // ->withInput();
            }
        //  dd($request->all());
        ...
        {
            // dd($request->gateways);
            $gate = Gateway::findOrFail($request->gateways);
            // dd($gate);
            if(isset($gate))
            {
                    ...
                    if ($gate->id > 899) {
                      ...
                      if($request->hasFile('receipt')) {
                        ...
                        if (!in_array($image->getClientOriginalExtension(), ['jpg', 'jpeg', 'png'])) {
                          Session::flash('alert', 'Le format du fichier doit être de type jpg, jpeg, png');
                        ...
                        Session::flash('alert', 'Une image est requise');
                        return redirect()->back();
                      }
                      $dr->save();
                      Session::flash('success', 'Demande de dépôt envoyée avec succès');
                    }
                ...
                else
                {
                    ...
                    Session::flash('alert', 'Veuillez suivre la limite de dépôt');
                    return redirect()->back();
                }
            }
            else
            {
                return back()->with('alert', 'Veuillez sélectionner la passerelle de dépôt');
            }
        }
    ...
    public function depositConfirm()
    {
        ...
        if(is_null($data))
        {
            return redirect()->route('vendor.showDepositMethods')->with('alert', 'Demande de dépôt invalide');
        }
        if ($data->status != 0)
        {
            return redirect()->route('vendor.showDepositMethods')->with('alert', 'Demande de dépôt invalide');
        ...
        elseif ($data->gateway_id == 501)
        {
            ...
            if($data->btc_amo==0 || $data->btc_wallet=="")
            {
                ...
                if(!$resp)
                {
                    return redirect()->route('vendor.showDepositMethods')->with('alert', 'L\'API BLOCKCHAIN ​​A UN PROBLÈME. MERCI D\'ESSAYER PLUS TARD');
                }
        ...
        elseif($data->gateway_id ==502)
		{
            ...
            {
                return back()->with('alert', 'Échec du traitement');
            }
    ...
    //IPN Functions //////
                     ...
                     if (!$err)
                     {
                         switch ($_POST['m_status'])
                         {
                             case 'success':
                                 ...
                                 $this->userDataUpdate($ddd);
                                 $message = 'Déposé avec succès';
                                 $err = false;

                                 break;

                             default:
                                 $message .= "-l'état du paiement n'est pas un succès\n";
                                 $err = true;
                                 break;
                         }
                     }
     ...
     public function purchaseVogue($trx, $type)
     {
         ...
         if ($type == 'error') redirect()->route('home')->with('alert', 'Transaction échouée, Ref: ' . $trx);
         return redirect()->route('vendor.showDepositMethods')->with('success', 'Déposé avec succès!, Ref: ' . $trx);
