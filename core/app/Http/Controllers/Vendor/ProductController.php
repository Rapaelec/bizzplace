...
class ProductController extends Controller
{
    ...
    public function store(Request $request) {
      ...
      $vendor = Vendor::find(Auth::guard('vendor')->user()->id);
      //Verifie si le vendeurs dispose de produit
      if ($vendor->products == 0) {
        return "no_product";
      }
      ...
      $in = $request->only('title','keywords','price','price_min','description','quantity', 'quantity_min','offer_amount', 'flash_amount', 'flash_date', 'flash_interval');
      $in['vendor_id'] = $vendor_role = Auth::guard('vendor')->user()->id;
      $in['slug'] = $slug;
      $in['keywords'] = $request->keywords;
