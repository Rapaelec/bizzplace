...
@section('title', 'Gestion du panier')
@section('headertxt', 'Gestion du panier')
                      ...
                      <div class="bottom-content"><!-- bottom content -->
                          <div class="left-content-area">
                              <div class="coupon-code-wrapper">
                                  <a href="{{route('user.search')}}" class="boxed-btn btn" style="width: 200px;"> ALLER FAIRE DU SHOPPING </a>
                              </div>
                          </div>
                          <div class="right-content-area">
                              <div class="btn-wrapper">
                                  <button type="button" class="boxed-btn" onclick="updatecart(event);"> Mise à jour panier </button>
                                  <a class="boxed-btn" href="{{route('user.checkout.index')}}"> Procéder au Paiement </a>
                              </div>

                              <div class="cart-total">
                                <br>
                                  <h5>Sélectionnez une méthode d'expédition:</h5>
                                  <div class="row">
                                    <div class="col-md-12">
                                      <div class="form-check form-check-inline">
                                        ...
                                        <label class="form-check-label" for="inlineCheckbox1">Dans la ville </label>
                                      </div>
                                      <div class="form-check form-check-inline">
                                        ...
                                        <label class="form-check-label" for="inlineCheckbox2">Autour de la ville {{$gs->main_city}}</label>
                                      </div>
                                      <div class="form-check form-check-inline">
                                        ...
                                        <label class="form-check-label" for="inlineCheckbox3">Partout dans le monde</label>
                                      </div>
                                    </div>
                                  </div>
                                  <br>
                                  ...
                                  <div class="row">
                                    <div class="col-md-12">
                                      <select class="form-control" name="payment_method" id="paymentMethod" onchange="calcTotal(this.value)">
                                        <option value="1" {{session('paymentMethod')==1 || !session()->has('paymentMethod')?'selected':''}}>Cash à la livraison</option>
                                        <option value="2" {{session('paymentMethod')==2?'selected':''}}>Avance</option>
                                      </select>
                                    </div>
                                  </div>
                                  <br>
                                  @if (Auth::check())
                                    ...
                                    @if ($countCoupon > 0)
                                      ...
                                      Le coupon est déjà appliqué! Ce code promo est valable jusqu'au {{date('jS F, Y', strtotime($gs->valid_till))}}
                                      </div>
                                    @endif
                                  @endif

                                  <h3 class="title">Totaux du panier</h3>
                                  <ul class="cart-list">
                                      <li>Sous total <span class="right" id="subtotal">{{$gs->base_curr_symbol}} {{getSubTotal($sessionid)}}</span></li>
                                      <li>Frais de port <span class="right" id="shippingCharge"></span></li>
                                      <li>Taxe <span class="right">{{$gs->tax}}%</span></li>
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.status == 'shortage') {
              ...
              toastr["error"]("<strong>Désolé!</strong> le vendeur "+ data.quantity +" articles laissés pour ce produit!");
            }
          }
        });
      }
        ...
        $.ajax({
          ...
          success: function(data) {
            ...
            if (data.status == "success") {
              ...
              // Fire toastr
              toastr["success"]("<strong>Succès!</strong> Panier mis à jour avec succès!");
