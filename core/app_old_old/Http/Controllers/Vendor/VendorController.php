...
class VendorController extends Controller
{
    public function dashboard() {
      ...
      $orderids = Order::join('orderedproducts', 'orders.id', '=', 'orderedproducts.order_id')->select('orders.id')->groupBy('orders.id')->where('vendor_id', 1)->orderBy('id', 'DESC')->limit(10)->first();
      $orderidarr = [];
    ...
    $data['evenements'] = Evenement::when($category, function ($query, $category) {
                      return $query->where('category_id', $category);
                  })
                  ->when($subcategory, function ($query, $subcategory)  {
                      return $query->where('subcategory_id', $subcategory);
    ...
    public function transactions() {
      $data['transactions'] = Transaction::where('vendor_id', 1)->orderBy('id', 'DESC')->paginate(10);
    ...
    public function couponlog(Request $request) {
      ...
      if ($request->to_date || $request->from_date || $request->order_id) {
        ...
        $data['logs'] = Orderedproduct::where('vendor_id', 1)->whereNotNull('coupon_amount')
        ...
        $data['ctotal'] = Orderedproduct::where('vendor_id', 1)->whereNotNull('coupon_amount')
                                        ->whereDate('created_at', '>=', $fromDate)->whereDate('created_at', '<=', $toDate)
                                        ->when($orderid, function ($query, $orderid) {
                                            return $query->orWhere('order_id', $orderid+100000);
                                        })
        ...
        $data['logs'] = Orderedproduct::where('vendor_id', 1)->whereNotNull('coupon_amount')->orderBy('id', 'DESC')->paginate(10);
        $data['ctotal'] = Orderedproduct::where('vendor_id', 1)->whereNotNull('coupon_amount')->sum('coupon_amount');
      }
