        ...
        <div class="order-track-page-content">
          <div class="track-order-form-wrapper">
            <div class="col-sm-12" style="text-align:left">
              <h2 style="color:azure;font-size:320%" class="text-center"> Commande de chez {{ $vendors->shop_name }}</h2>
            </div>
          </div>
        </div>
      </div>
                            ...
                            <tbody>
                              @foreach ($orders as $key => $order)
                                <tr>
                                    ...
                                    <td class="padding-top-40">
                                      <a class="btn btn-primary" data-toggle="tooltip" data-placement="bottom" href="{{ ($order->shipping_status == 2 && $order->approve == 1) ? route('user.view_facture',[$order->id,$order->vendor_id]) : route('user.orderdetails', [$order->id,$order->vendor_id]) }}">
                                        <i class='fa fa-{{ ($order->shipping_status == 2 && $order->approve == 1) ? "print" : "eye" }}' title="{{ ($order->shipping_status == 2 && $order->approve == 1) ? 'Imprimer la facture' : 'Voir la commande' }}"></i>
