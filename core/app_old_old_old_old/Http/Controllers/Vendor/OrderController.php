  ...
  public function orders(Request $request) {
    ...
    public function acceptOrder(Request $request) {
      ...
      foreach($orders as $order){
        $order->approve = 1;
        $this->var = $this->var+$order->approve;
        $order->save();
      }
      send_email($order->user->email, $order->user->first_name,$order->user->phone,$order->user->email, 'Commande acceptée', "Votre commande a été acceptée.<p><strong>Numéro de commande: </strong>$order->unique_id</p><p><strong>Détails de la commande: </strong><a href='".url('/')."/".$order->id."/détails de la commande'>".url('/')."/".$order->id."/détails de la commande"."</a></p>");
      return "success";
    }
